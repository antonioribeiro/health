...
export default {
  ...
  methods: {
    ...
    checkResource(resource) {
      ...
      axios.get('/health/resources/' + resource.slug + '?flush=1').then(function(response) {
      ...
      getAllTargets(type) {
          ...
          _.each(this.resources, function(resource) {
              _.each(resource.targets, function(target) {
                  if (type == 'all' || (type == 'failing' && target.result && !target.result.healthy) || (type == 'healthy' && target.result && target.result.healthy)) {
                      targets.push(target)
                  }
              });
          })
    ...
    computed: {
        ...
        isLoading() {
            return _.reduce(this.resources, function(current, resource) {
